...
export default function EducationFormModal({
  ...
  useEffect(() => {
    ...
    const loadDetail = async () => {
      ...
      try {
        const result = (await educationAPI.getEducationById(itemId)) as {
          success?: boolean;
          status?: string;
          data?: EducationItem;
          errMessage?: string;
  ...
  const handleSave = async () => {
    ...
    await withLoading(async () => {
      if (isEdit && itemId != null) {
        const payload: UpdateEducationPayload = payloadBase;
        const result = (await educationAPI.updateEducation(
          itemId,
          payload
        )) as {
          success?: boolean;
      ...
      const payload: CreateEducationPayload = payloadBase;
      const result = (await educationAPI.createEducation(payload)) as {
        success?: boolean;
        status?: string;
        errMessage?: string;
        message?: string;
      ...
      if (!result || result.status === "failed" || result.success === false) {
        await popup.error(
          "Create failed",
          result?.errMessage || result?.message || "Unable to create education"
        );
        return;
      }
      saved = true;
    }, isEdit ? "Saving changes..." : "Creating education...");
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <RichTextEditor
                  ...
                  placeholder="Education description in Thai..."
                />
              </div>
              ...
              <div>
                ...
                <RichTextEditor
                  ...
                  placeholder="Education description in English..."
                />
              </div>
