...
function sortByDisplayOrder(list: ProjectItem[]) {
...
export default function ProjectMain() {
  ...
  const handleReorder = async (visibleOrdered: ProjectItem[]) => {
    if (!canEdit) return;
    ...
    let saved = false;
    await withLoading(async () => {
      const result = (await projectAPI.reorderProjects(orderedIds)) as {
        success?: boolean;
        status?: string;
        data?: ProjectItem[];
        errMessage?: string;
      ...
      if (!result || result.status === "failed" || result.success === false) {
        ...
        await popup.error(
          "Reorder failed",
          result?.errMessage || result?.message || "Unable to reorder projects"
        );
        return;
      }
